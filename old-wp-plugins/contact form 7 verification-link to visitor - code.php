...
/*    wp_mail($senders_email_address , 'Verify your email address',
	...
	wp_mail($senders_email_address, 'Verify your email address',
            "Hi,\n\nThanks for your your recent submission on In order for your submission to be processed, please verify this is your email address by clicking on the following link:\n\n" . 
            get_site_url() . "/wp-admin/admin-post.php?action=wpcf7ev&email-verification-key={$random_hash}" . "\n\nThanks.");
        ...
        if(!empty($verification_key))
        {
            ...
            else
            {
                ...
                wp_mail($cf7_mail_fields['recipient'], $cf7_mail_fields['subject'], $cf7_mail_fields['body'],'', $mail_attachments);
					//print_r($cf7_mail_fields[additional_headers]);
					$add_header=explode(": ", $cf7_mail_fields[additional_headers]);
					$visitor_email=$add_header[1];
					$red_add=get_site_url().'/thank-you';
				?>
				<?php
				//echo get_site_url().'/thank-you?unkid=';
				$encid= bin2hex($visitor_email); // result: 48656c6c6f
				$add=$red_add.'?unid='.$encid;
				
                // display a confirmation message then redirect back to the homepage after 8 seconds
                echo('<h2 style="text-align:center;">Verification key accepted.</h2>' .'<p style="text-align:center;">If you are not redirected back to the Thank you page in 8 seconds, <a href="' .$add. '">click here</a>.</p>' .
                     '<script> setTimeout(function () { window.location.href = "' .$add. '"; }, 8000); </script>');
