...
class pcfme_PluginUpdateChecker_2_2 {
	...
	public function displayManualCheckResult() {
		if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->slug) ) {
			...
			if ( $status == 'no_update' ) {
				$message = 'Woocommerce Easy Checkout Fields Editor plugin is up to date.';
			} else if ( $status == 'update_available' ) {
			
			    if (($this->is_code_valid) == "active") {
					$message = 'A new version of Woocommerce Easy Checkout Fields Editor plugin is available';
				} else {
					$message= sprintf(
